...
function OnboardingTab() {
  ...
  async function handleToggleStatus(task: OnboardingTask) {
        ...
        return (
          <Card key={emp.employee.id}>
            <CardHeader>
              <div>
                <CardTitle>
                  {emp.employee.name} &mdash; 온보딩
                </CardTitle>
                <p className="mt-sp-1 text-sm text-text-tertiary">
                  {emp.employee.department} &middot;{" "}
                  {emp.employee.position} &middot; 입사일{" "}
                  {formatHireDate(emp.employee.hireDate)}
                </p>
              </div>
