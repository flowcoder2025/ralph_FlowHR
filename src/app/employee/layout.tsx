...
const EMPLOYEE_NAV: NavSection[] = [
  {
    items: [
      ...
      { id: "payslips", label: "급여", href: "/employee/payslips" },
    ...
    <div className="flex h-screen overflow-hidden bg-surface-canvas">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 flex md:hidden border-t border-border bg-surface-primary">
        ...
        <button
          type="button"
          onClick={() => signOut({ callbackUrl: "/login" })}
          className="flex flex-1 flex-col items-center justify-center gap-0.5 py-2 text-[10px] font-medium transition-colors min-h-[56px] text-text-tertiary hover:text-status-danger"
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
