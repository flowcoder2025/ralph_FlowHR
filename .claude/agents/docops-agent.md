...
사용자가 세션에서 명시적으로 합의한 다음 작업은 자동 정렬보다 우선한다.
state.md "다음 작업" 섹션에 "사용자 합의 작업"과 "자동 정렬 작업"을 구분하여 기록한다.

```
0순위: 사용자 합의 작업 (세션에서 명시적으로 확정한 다음 작업)
1순위: 시스템/인프라 미완료 (도구 자체가 완성 안 됨 → 이걸로 작업할 수 없음)
2순위: 기존 기능 버그/미완료 (동작하지만 불완전)
3순위: 신규 기능 (새로운 것)
```
사용자가 "다음에 이걸 하자"고 확정한 작업은 시스템/인프라보다도 먼저 표시한다.
도구가 완성 안 됐는데 그 도구로 작업하면 안 되므로, 자동 정렬에서는 시스템 작업이 먼저.
